...
public class MySQLQueryGenerator extends RDBMSQueryGenerator {
	...
	@Override
	public Query generateQuery(Chart chart) throws BDAException, Exception {

		MySQLQueryGenerator sql = new MySQLQueryGenerator(); 
		//String tableName= chart.getTableColumns().keySet().iterator().next();
		...
		table.addColumnsToSelect(x_axis,x_axis);
		table.addColumnsToSelect(y_axis,y_axis_agg_fn,y_axis);
		if(z_axis != null)
			table.addColumnsToSelect(z_axis,z_axis);

		sql.setNoOfRecords(chart.getNoOfRecords());   

		sql.addTable(table);
		sql.criterias = chart.getFilterConditions();
		if(!y_axis_agg_fn.equalsIgnoreCase(BDAConstants.NONE)){
			sql.addGroupByColumn(table, x_axis);
			if(z_axis != null)
				sql.addGroupByColumn(table, z_axis);
		}
